...
class MonitorConfig:
    ...
    def to_xrandr_cmd(self) -> str:
        """Generate an xrandr command line for this monitor."""
        if not self.enabled:
            return f"xrandr --output {self.name} --off"

        parts = [f"xrandr --output {self.name}"]
        ...
        # Position
        if self.position_mode == PositionMode.EXPLICIT:
            parts.append(f"--pos {self.x}x{self.y}")
    ...
    def to_hyprland_line(self) -> str:
        ...
        if self.mirror_of:
            extras.append(f"mirror, {self.mirror_of}")
...
class WorkspaceRule:
    ...
    def to_sway_line(self) -> str:
        ...
        if self.workspace and self.monitor:
            return f"workspace {self.workspace} output {self.monitor}"
        return ""
...
class Profile:
    ...
    def generate_config(self) -> str:
        ...
        lines: list[str] = []
        lines.append("# Generated by Monique")
        lines.append("")
        ...
        if self.workspace_rules:
            ...
            for w in self.workspace_rules:
                lines.append(w.to_hyprland_line())
        lines.append("")
        return "\n".join(lines)

    def generate_sway_config(self) -> str:
        """Generate the full monitors.conf content for Sway."""
        blocks: list[str] = ["# Generated by Monique"]
        for m in self.monitors:
            blocks.append(m.to_sway_block())
        ws_lines = [w.to_sway_line() for w in self.workspace_rules if w.to_sway_line()]
        if ws_lines:
            blocks.append("\n".join(ws_lines))
        return "\n\n".join(blocks) + "\n"
    ...
    def generate_xsetup_script(self) -> str:
        ...
        lines = ["#!/bin/sh", "# Generated by Monique"]
        for m in self.monitors:
